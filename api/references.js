...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "search-inputs") {
      ...
      if (orseBases.length) {
        try {
          ...
          orse = [...inputGroups.flat().slice(0,45).map(item=>({...item,tipoItem:"INSUMO"})),
            ...compositionGroups.flat().slice(0,25).map(item=>({...item,tipoItem:"COMPOSICAO"}))];
    ...
    if (/budget_reference_/i.test(message) && /does not exist|schema cache/i.test(message)) {
      return res.status(503).json({ error: "Execute a atualização do schema.sql no Supabase antes de usar as referências." });
    }
