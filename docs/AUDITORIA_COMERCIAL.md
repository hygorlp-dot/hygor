...
| **Funil & Jornada do cliente** (`com_funil`, `com_jornada`) | **7,5** | Duas visualizações kanban genuinamente boas — a "Jornada" avança os cards sozinha conforme o sistema muda a etapa do lead (proposta enviada, contrato assinado…), ordenando por urgência dentro de cada fase. Mas `moverLead` (arrastar card) grava com `setCom` puro, sem checar o resultado nem mostrar erro se a gravação falhar — ver achado de funcionamento #2. |
| **Leads & Indicações** (`com_leads`, `com_indicacoes`) | **7,5** | O painel "Indicações" é um diferencial real (ranking de quem indica, taxa de indicação por obra entregue, NPS de entrega, onde o funil vaza) — nenhum CRM genérico de mercado tem isso pronto, é modelado para o motivo real da ARCD vender (indicação). Mas `salvarLead`/`excluirLead`/`salvarPerda` não têm trava contra duplo clique, e há dois `window.confirm` nativos. |
| **Agenda, Reuniões e Tarefas** (`com_agenda`, `com_reunioes`, `com_tarefas`) | **7,5** | Boa cobertura (lista + calendário mensal, criação automática da primeira tarefa ao criar lead, registro de resumo obrigatório para confirmar execução de reunião). `salvarAtividade`/`salvarReuniao` sem trava contra duplo clique. |
| **Propostas & Negociações** (`com_propostas`, `com_negociacoes`) | **4,5** | A tela em si tem regra de negócio real (`salvarProposta` bloqueia avanço sem lead/objeto/valor e aplica limite de desconto por usuário) — mas o item do menu lateral **"Propostas e contratos" abre em branco** (Achado 0, abaixo); só é alcançável hoje por um atalho não óbvio no dashboard (`onTab("com_propostas")` no card "Propostas enviadas"). Nota rebaixada por causa disso, não pela lógica interna. `statusProposta` (que também **gera contrato automaticamente** ao aceitar) grava com `setCom` puro sem checar sucesso; `pdfProposta` reinventa um gerador de HTML próprio em vez de reusar o padrão compartilhado. |
| **Contratos** (`com_contratos`) | **7** | Split real dentro da mesma aba: `finalizarContrato` (a ação mais crítica — ativa o contrato, cria obra, cliente, venda, comissão, medições e reuniões de kickoff/pós-venda) é exemplar — trava contra duplo clique, `try/finally`, comando versionado auditável, validação de 9 pré-condições, sem duplicar cálculo financeiro (ver seção de DRE). Alcançável pelo atalho "Sem assinatura" do dashboard (funciona), mas não tem item próprio no menu novo — está embutido em "Propostas e contratos", que está quebrado. As ações "ENVIAR" e "REGISTRAR ASSINATURA" do mesmo card usam `setCom` puro sem checar sucesso, e `salvarContrato` (rascunho) não tem trava contra duplo clique. |
| **Clientes & Parceiros** (`com_clientes`, `com_parceiros`) | **8** | Ponto alto do CRUD: `salvarCliente` valida CPF/CNPJ de verdade (dígito verificador), exige campos diferentes para PF/PJ (inclusive dados do representante legal e do cônjuge quando aplicável), e `pendenciasCliente` calcula ao vivo o que falta para o cadastro ficar "contratualmente completo" — uma checklist visual por cliente, sem esperar o usuário descobrir na hora de gerar o contrato. Falta só a trava contra duplo clique, como o resto do módulo. Não tem item de menu dedicado nem atalho de dashboard confirmado — só alcançável de dentro do fluxo de contrato. |
| **Metas & Comissões** (`com_metas`) | **4** | O item de menu **"Gestão comercial" também abre em branco** (Achado 0) — e mesmo depois de corrigido o bug de rota, ele só levaria a Relatórios (`com_relatorios`), nunca a Metas: o mapeamento atual de `commercialView` não tem nenhuma entrada para `com_metas` sob a nova navegação. Metas com barra de progresso por vendedor/período funcionam quando alcançadas (só por link direto hoje inexistente no dashboard). Comissões é o ponto fraco à parte: gerada automaticamente na ativação do contrato com uma taxa mágica de 1% quando não há parceiro de indicação (achado de bug #5), e nunca sai do status `"prevista"` — não existe em lugar nenhum do código um caminho para marcar comissão como paga, nem ela é lida pelo `ledger.js`/DRE. |
| **Perdas** (`com_perdas`) | **7,5** | Simples e funcional: motivo obrigatório, ranking por motivo, reativação quando a data chega. Baixo risco por ser baixo volume de escrita. |
| **Imobiliário — Venda de imóveis** (`com_real_estate`, componente `RealEstateCommercial.jsx`) | **8** | O módulo mais rico tecnicamente: dashboard de VGV com gráficos (disponível/vendido/reservado/negociação), auditoria de preço e status por unidade (`updateUnitWithAudit` guarda todo histórico), expiração automática de reserva liberando a unidade, trava de venda abaixo do mínimo exigindo aprovação de admin, dossiê de documentos por ativo. Bem testado (`real-estate.test.js`, 5 casos). Perde pontos porque o componente não recebe `showToast` como prop — por isso usa `window.alert`/`window.confirm` nativos, e não por escolha de design. |

**Nota geral do módulo: 6,7/10** (seria ~7,6 se não fosse o Achado 0 —
duas telas do menu novo em branco derrubam a média sozinhas). O ponto mais
forte do Comercial é exatamente o ponto mais crítico para o dinheiro: a
