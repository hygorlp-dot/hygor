...
export const CLIENT_PORTAL_DATA_INVENTORY = Object.freeze([
  ...
  { domain:"resumo do caixa da obra", source:"projectCashSummaries", fields:["id","totalContributions","totalExpenses","balance","asOf"], classification:"restricted_by_profile", profiles:["viewProjectCash"], publication:"habilitação explícita da obra + status=published; sem banco, PIX ou conciliação" },
  { domain:"movimentação do caixa da obra", source:"projectCashMovements", fields:["id","date","type","category","description","amount","balance"], classification:"restricted_by_profile", profiles:["viewProjectCash"], publication:"habilitação explícita da obra + status=published; sem comprovante bancário" },
  { domain:"nota fiscal compartilhada", source:"clientInvoices", fields:["id","type","number","issuedAt","dueDate","supplierName","description","category","grossAmount","netAmount","status"], classification:"restricted_by_profile", profiles:["viewProcurement"], publication:"habilitação explícita da obra + status=published; sem chave, CNPJ, retenções ou divergências internas" },
  { domain:"pedido de compra compartilhado", source:"clientPurchaseOrders", fields:["id","number","date","expectedAt","supplierName","status","total","items[description,unit,quantity,receivedQuantity,unitPrice]"], classification:"restricted_by_profile", profiles:["viewProcurement"], publication:"habilitação explícita da obra + status=published; sem pagamentos, conciliação ou análise interna" },
  { domain:"cotação compartilhada", source:"clientQuotations", fields:["id","date","status","material","unit","quantity","proposals[supplierName,unitPrice,total,leadTimeDays,selected]"], classification:"restricted_by_profile", profiles:["viewProcurement"], publication:"habilitação explícita da obra + status=published; sem dados fiscais ou contato do fornecedor" },
