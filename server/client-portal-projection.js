...
const measurementItem = item => ({ id:string(item.id), number:string(item.number || item.codigo), period:string(item.period || item.competencia), description:string(item.description || item.descricao), status:string(item.status), percentage:number(item.percentage || item.percentual), amount:number(item.clientAmount), dueDate:string(item.dueDate || item.vencimento), publishedAt:string(item.publishedAt), documentUrl:string(item.clientDocumentUrl) });
const paymentItem = item => ({ id:string(item.id), description:string(item.description), amount:number(item.amount), dueDate:string(item.dueDate), status:string(item.status), receiptUrl:string(item.clientReceiptUrl) });
