...
const C = {
  ...
  border:   "#C8C2B6",     // Borda visível em fundo branco &larr; REFORÇADA
...
.nav-item{
  ...
  padding:7px 10px 7px 12px; margin-bottom:1px;
  background:transparent; border:0; border-radius:0 7px 7px 0;
  cursor:pointer; text-align:left;
  color:${C.muted};
...
// Feriados oficiais são função pura do ano &rarr; cache permanente
...
// espelhando o fluxo típico (estudo &rarr; aprovações &rarr; executivo &rarr; obra).
...
const normalizeData = incoming => {
  ...
  return {
    ...
    // Categorias de despesa do DRE, editaveis pelo operador. Se vazio, o app
    // usa as padrao (OUTRAS_CATEGORIAS). Cada uma tem v (chave) e l (rotulo).
    categoriasDesp: Array.isArray(d.categoriasDesp) ? d.categoriasDesp.map(c => ({
      v: c.v || uid(), l: c.l || "Categoria", fixa: !!c.fixa,
    })) : [],
    ...
    rdos: Array.isArray(d.rdos) ? d.rdos.map(r => ({
      ...
      // progresso acumulado total (progressoAte). Guardamos os dois para a
      // medicao poder somar dias trabalhados.
      servicos: Array.isArray(r.servicos) ? r.servicos.map(s => ({
        tarefaId:     s.tarefaId     || "",
        etapaId:      s.etapaId      || "",
        descricao:    s.descricao    || "",
        progressoAte: Math.max(0, Math.min(100, Number(s.progressoAte || 0))),
        obs:          s.obs          || "",
      })) : [],
      // Mao de obra do dia. Pode vir do ponto (presencas) ou ser digitada
      // por funcao (efetivo), como manda a boa pratica de RDO.
      efetivo: Array.isArray(r.efetivo) ? r.efetivo.map(e => ({
        funcao: e.funcao || "", quantidade: Number(e.quantidade || 0),
      })) : [],
      // Presencas lancadas direto no diario (integra o ponto sem trocar de aba).
      presencas: Array.isArray(r.presencas) ? r.presencas.map(p => ({
        empId: p.empId || "", status: p.status || "presente",  // presente | meio | falta
      })) : [],
      equipamentos: Array.isArray(r.equipamentos) ? r.equipamentos.map(eq => ({
        nome: eq.nome || "", quantidade: Number(eq.quantidade || 0),
      })) : [],
...
function Ic({ n, s = 16, color }) {
  // Icones em SVG (feather-style). Antes eram emojis, que o build do Vercel
  // apaga - por isso sumiam na tela. SVG e ASCII puro: aparece sempre, tem
  // contraste, herda a cor via --ic-color ou a prop color.
  const P = {
    ...
    ia:       "M12 2a3 3 0 0 0-3 3 3 3 0 0 0-3 3v1a3 3 0 0 0 0 6v1a3 3 0 0 0 3 3 3 3 0 0 0 6 0 3 3 0 0 0 3-3v-1a3 3 0 0 0 0-6V8a3 3 0 0 0-3-3 3 3 0 0 0-3-3z M12 2v20",
    brain:    "M12 2a3 3 0 0 0-3 3 3 3 0 0 0-3 3v1a3 3 0 0 0 0 6v1a3 3 0 0 0 3 3 3 3 0 0 0 6 0 3 3 0 0 0 3-3v-1a3 3 0 0 0 0-6V8a3 3 0 0 0-3-3 3 3 0 0 0-3-3z M12 2v20",
    robot:    "M12 2v3 M8 5h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2z M9 11h.01 M15 11h.01 M4 12H2 M22 12h-2",
    ...
    terc:     "M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2 M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z M22 21v-2a4 4 0 0 0-3-3.87 M16 3.13a4 4 0 0 1 0 7.75",
    week:     "M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z M16 2v4 M8 2v4 M3 10h18",
  ...
  const d = P[n];
  if (!d) {
    // fallback discreto: um ponto, nao um traco solto
    return (
      <span aria-hidden="true" style={{
        width:s+4, height:s+4, display:"inline-flex", alignItems:"center",
        justifyContent:"center", color: color || `var(--ic-color, ${C.yellow})`,
      }}>
        <svg width={s*0.4} height={s*0.4} viewBox="0 0 24 24"><circle cx="12" cy="12" r="12" fill="currentColor"/></svg>
      </span>
    );
  }
  return (
...
// Seta (chevron) que gira ao abrir. Tambem SVG.
function ChevronBaixo({ s = 14 }) {
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none"
...
function Dashboard({ data, onTab, ultimaSync }) {
    ...
    <button onClick={()=>tab&&onTab(tab)} className="lift-card" style={{
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",
                      width:30,height:30,borderRadius:9,background:`${color}1A`,flexShrink:0}}>
  ...
  return (
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {/* Carimbo de sincronizacao: quando o app leu/gravou por ultimo */}
      {ultimaSync && (
        <div style={{ display: "flex", alignItems: "center", gap: 7,
                      padding: "8px 12px", background: `${C.green}0E`,
                      border: `1px solid ${C.green}33`, borderRadius: 9 }}>
          <span style={{ width: 7, height: 7, borderRadius: "50%", background: C.green,
                         flexShrink: 0, boxShadow: `0 0 0 3px ${C.green}22` }} />
          <span style={{ fontSize: 11.5, color: C.subtle }}>
            Informacoes atualizadas as {ultimaSync.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}
            {" "}de {ultimaSync.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })}
          </span>
        </div>
      )}
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden",marginBottom:8}}>
          ...
          {dre.faturamento>0&&(
            <div style={{padding:"8px 14px",borderTop:`1px solid ${C.line}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <button onClick={()=>onTab("dre")} style={{background:"transparent",border:`1px solid ${C.border}`,color:C.muted,padding:"3px 10px",borderRadius:8,cursor:"pointer",fontSize:10,fontWeight:700}}>Ver DRE &rarr;</button>
            </div>
          )}
        </div>
      </div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                {pending.length > 0 ? (
                  ...
                  <p style={{ fontSize:12, color:C.green, marginTop:3 }}>ok Todos os {activeTerc.length} terceirizados já foram pagos</p>
                )}
              </div>
              <div style={{ background:pending.length>0?C.orange:C.green, color:C.ink, borderRadius:999, padding:"4px 10px", fontFamily:"'Inter Display','Inter',sans-serif",fontWeight:800, fontSize:18, letterSpacing:1 }}>
                {pending.length>0?pending.length:"ok"}
              </div>
            </div>
          </button>
        );
          ...
          {(data.outrasDesp||[]).filter(d=>d.competencia===`${year}-${String(month+1).padStart(2,"0")}`).map(d=>(
            <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 14px",borderBottom:`1px solid ${C.line}22`}}>
              <div>
                ...
                <p style={{fontSize:11,color:C.muted}}>{rotuloCategoria(data, d.categoria)}  {data.obras.find(o=>o.id===d.obraId)?.name||"-"}</p>
              </div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <div style={{flex:1}}>
              ...
              {obra.contractStart && (
                <p style={{fontSize:11,color:C.muted,marginTop:4}}>
                   {fmtDateFull(obra.contractStart)}{obra.contractEnd && ` &rarr; ${fmtDateFull(obra.contractEnd)}`}
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                    ...
                    {m.tipo==="percentual"&&<span style={{fontSize:10,color:C.blue,fontWeight:600,background:`${C.blue}10`,padding:"2px 8px",borderRadius:4}}>{prevPct}%&rarr;{m.percentualAcumulado}% (+{m.percentualPeriodo.toFixed(1)}%)</span>}
                  </div>
            ...
            {form.tipo==="mensal_fixo"&&(
              <div style={{background:C.surface,border:`1.5px solid ${C.border}`,borderRadius:8,padding:"10px 14px"}}>
                <p style={{fontSize:12,color:C.text,fontWeight:700,marginBottom:6}}>
                  Parcela mensal fixa
                  {obra.parcelaMensal>0&&<span style={{color:C.yellow,marginLeft:8}}>&rarr; {fmt(obra.parcelaMensal)}</span>}
                </p>
                ...
                {Number(form.percentualAcumulado)>0 && obra.contractValue>0 && (() => {
                  ...
                  return (
                    <div style={{background:`${C.yellow}15`,borderRadius:6,padding:"8px 12px"}}>
                      <p style={{fontSize:11,color:C.muted}}>Anterior: <strong>{prev}%</strong> &rarr; Este período: <strong>+{periodo.toFixed(1)}%</strong></p>
  ...
  const saveEmp = () => {
    ...
    if (before && before.obra !== payload.obra) {
      changeLog.push({
        ...
        fromId: before.obra,   // &larr; ID gravado
        toId: payload.obra,    // &larr; ID gravado
...
function WorkerMovementModal({ data, update, showToast, employee, initialMode = "transfer", onClose }) {
  ...
  const saveTransfer = () => {
    ...
    const changeLog = [...data.changeLog, {
      ...
      from,
      to,
      fromId: employee.obra,  // &larr; ID gravado para busca confiável
      toId: newObra,          // &larr; ID gravado para busca confiável
...
function FluxoCaixa({ data }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,padding:14,borderRadius:18}}>
        <p style={{fontSize:11,fontWeight:900,color:C.muted,textTransform:"uppercase",letterSpacing:.8,marginBottom:12}}>Receita x Saída por mês</p>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderTop:`3px solid ${C.orange}`, padding:"14px 16px", borderRadius:16 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            <div style={{ textAlign:"center" }}>
              ...
              <p style={{ fontSize:11, color:C.muted, marginTop:2 }}>
                Semana: {fmtDateFull(weekStart)} &rarr; {fmtDateFull(weekEnd)}
              </p>
            </div>
            <button onClick={()=>setWeekOffset(w=>w+1)} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.text, padding:"6px 14px", cursor:"pointer", borderRadius:10, fontWeight:900, fontSize:18 }}></button>
          </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
            <div>
              ...
              <p style={{fontSize:11,color:C.muted,marginTop:2}}>
                ...
                {fmtDateFull(r.admissao)} &rarr; {fmtDateFull(r.demissao)}
              </p>
      ...
      <div style={{width:"100%",maxWidth:340}}>
        ...
        {isFirst ? (
          ...
          <div style={{background:C.surface,border:`1.5px solid ${C.border}`,borderTop:`3px solid ${C.yellow}`,borderRadius:10,padding:20}}>
            ...
            {/* Teclado numérico */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {[1,2,3,4,5,6,7,8,9,"",0,"&larr;"].map((d,i)=>(
                <button key={i} onClick={()=>d===""?null:d==="&larr;"?handleBackspace():handleDigit(String(d))} style={{
                  background:d===""?"transparent":C.bg,
                  border:`1.5px solid ${d===""?"transparent":C.border}`,
                  borderRadius:8, padding:"14px 0",
                  fontFamily:"'Inter Display','Inter',sans-serif",
                  fontSize:18,fontWeight:700,color:d==="&larr;"?C.muted:C.text,
...
const useBreakpoint = () => {
  ...
  return {
    ...
    // Grade responsiva: cols(1,2,4) &rarr; 1 col no celular, 2 no tablet, 4 no desktop
...
const aplicarRollup = (tarefas, orc) => {
  const etapaDe = {};
...
// ==============================================================
const fisicoFinanceiro = (tarefas) => {
...
// ==============================================================
const caminhoCritico = (tarefas, cal) => {
...
const construirArvore = (etapas, itens) => {
  ...
  // Índice etapa &rarr; itens, montado UMA vez. Antes, cada etapa varria a lista
...
// Achata a árvore na ordem de leitura (etapa &rarr; sub-etapas &rarr; itens diretos)
...
function Orcamento({ data, update, showToast }) {
  ...
  const extrair = (rows, layout) => {
    ...
    for (let r = headerRow + 1; r < rows.length; r++) {
      ...
      out.push({
        ...
        codigo:    codigoRaw.replace(/\.0$/, ""),   // "97141.0" &rarr; "97141"
        descricao,
  ...
  const importarXLSX = async (file) => {
    ...
    try {
      ...
      // Auto-detecção falhou &rarr; mapeamento manual
  ...
  const confirmarMapeamento = () => {
    ...
    const itens = rows.map(r => {
      ...
      return {
        ...
        precoDes:  0,
        precoNao:  preco,   // mapeamento manual &rarr; coluna única vira "não desonerado"
      };
  ...
  const resultados = useMemo(() => {
    ...
    const score = (i) => {
      let s = 0;
      // 1. Código exato &rarr; topo absoluto
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {[
              ...
              <Sel key={k} label={l} value={colMap[k]} onChange={v=>setColMap(m=>({...m,[k]:v}))}
                options={[{v:"",l:"- Selecionar coluna -"}, ...mapModal.headers.map((h,i)=>({v:String(i), l:`${h}  &rarr;  ${String(mapModal.rows[0]?.[i] ?? "").slice(0,32)}`}))]}
              />
            ))}
            <div style={{display:"flex",gap:8}}>
...
// "20240315120000[-3:BRT]" | "20240315"  &rarr;  "2024-03-15"
const ofxData = (s) => {
...
const sugerirMedicoes = (tr, data) => {
  ...
  (data.medicoes || []).filter(m => !m.recebido).forEach(m => {
    ...
    else return;                                 // recebeu MAIS que o previsto &rarr; não é esta
...
const valorEsperadoPonto = (data, emp, days) => {
  let bruto = 0, beneficios = 0, diasP = 0, diasM = 0;
  const year = days[0] ? Number(days[0].slice(0, 4)) : new Date().getFullYear();
  const holidays = getPayrollHolidays(data, year);
  days.forEach(d => {
    ...
    if (st === "P") { bruto += Number(emp.dailyRate || 0); beneficios += Number(emp.vtDaily || 0) + Number(emp.vrDaily || 0); diasP++; }
    else if (st === "M") { bruto += Number(emp.dailyRate || 0) * .5; beneficios += (Number(emp.vtDaily || 0) + Number(emp.vrDaily || 0)) * .5; diasM++; }
  });
...
const sugerirPagamentoMaoObra = (tr, data, days) => {
  ...
  (data.employees || []).filter(e => e.active !== false).forEach(emp => {
    ...
    if (dif < 0.01)              { score += 100; motivos.push("valor exato do ponto"); }
    ...
    if (score >= 40) {
      ...
      cands.push({
        ...
        divergencia,
        pagoATerceiro: !!(emp.pixHolder && prim(emp.pixHolder) &&
                          desc.includes(prim(emp.pixHolder)) &&
...
//   - Cotação      &rarr; compara fornecedores e prova que você comprou bem
//   - Pedido       &rarr; registra o compromisso
//   - Recebimento  &rarr; gera a ENTRADA no estoque da obra (o elo físico)
//   - Histórico    &rarr; quanto você pagou nesse item, e com quem
// 
...
const fornecedoresParaMateriais = (data, materialIds) => {
  const mats = (data.materiais || []).filter(m => materialIds.includes(m.id));
  const catsNecessarias = [...new Set(mats.map(m => m.categoria).filter(Boolean))];
  const pedidos = data.pedidos || [];

  const scores = {};
  (data.fornecedores || []).filter(f => f.ativo !== false).forEach(forn => {
    let score = 0;
    const motivos = [];
    const catsAtende = (forn.categorias || []).filter(c => catsNecessarias.includes(c));
    ...
    materialIds.forEach(mid => {
      const comprou = pedidos.some(p => p.fornecedorId === forn.id && p.status !== "cancelado" &&
        (p.itens || []).some(i => i.materialId === mid && Number(i.qtdRecebida) > 0));
...
const linkWhatsApp = (telefone, texto) => {
...
// Painel da obra: comprado &rarr; recebido &rarr; aplicado &rarr; pago.
...
// "Executei 120 m de alvenaria" &rarr; quanto sai de cada insumo
...
function Compras({ data, update, showToast }) {
  ...
  return (
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {/* Cadeia comprado &rarr; recebido &rarr; aplicado &rarr; pago */}
      <div style={{display:"grid",gridTemplateColumns:cols(2,4,4),gap:8}}>
        {[
...
function ObraDetalhe({ data, obraId, onVoltar, onTab, update, showToast }) {
  ...
  const resumo = useMemo(() => {
    ...
    const equipe = (data.employees||[]).filter(e => e.active !== false && e.obra === obraId);
    ...
    //  Estoque 
    const saldos = calcSaldos((data.movEstoque||[]).filter(m => m.obraId === obraId));
    ...
    //  Pedidos em aberto 
    const pedAbertos = (data.pedidos||[])
      .filter(p => p.obraId === obraId && ["enviado","parcial"].includes(statusPedido(p)));

    //  Fase no quadro 
    const fase = (data.fases||[]).find(f => f.id === obra.faseId);

    return {
      equipe: equipe.length, presentesHoje,
      medicoes, aReceber, recebido, despesas,
  ...
  const [secoesAbertas, setSecoesAbertas] = useState({ financeiro: true, equipe: true });
  ...
  const atividade = useMemo(() => {
    const ev = [];
    (data.medicoes||[]).filter(m => m.obraId===obraId && m.recebido).forEach(m =>
      ev.push({ data:m.dataPagamento||m.dataVencimento, tipo:"",
                txt:`Recebido: ${m.descricao || "medição"}`, valor:Number(m.valorRecebido||0) }));
    (data.pedidos||[]).filter(p => p.obraId===obraId).forEach(p =>
      ev.push({ data:p.data, tipo:"", txt:`Pedido ${p.numero}`, valor:totalPedido(p) }));
    (data.movEstoque||[]).filter(m => m.obraId===obraId).slice(-20).forEach(m => {
      const mat = (data.materiais||[]).find(x => x.id === m.materialId);
      ev.push({ data:m.data, tipo: m.tipo==="entrada" ? "" : "",
                txt:`${m.tipo==="entrada"?"Entrada":"Consumo"}: ${m.qtd} ${mat?.unidade||""} de ${mat?.descricao||"-"}`,
                valor:0 });
    });
    (data.outrasDesp||[]).filter(d => d.obraId===obraId).forEach(d =>
      ev.push({ data:(d.competencia||"")+"-01", tipo:"", txt:d.descricao||d.categoria, valor:Number(d.valor||0) }));
  ...
  if (!obra || !resumo) {
    return (
      <div style={{padding:20,textAlign:"center"}}>
        ...
        <Btn v="ghost" onClick={onVoltar} full style={{marginTop:12}}>&larr; Voltar</Btn>
      </div>
    );
  }
  ...
  const Par = ({ l, v, c }) => (
    <div style={{display:"flex",justifyContent:"space-between",gap:8,marginTop:4}}>
      ...
      <span style={{fontSize:12,fontWeight:700,color:c||C.text,whiteSpace:"nowrap"}}>{v}</span>
    </div>
  );
  ...
  const Secao = ({ id, icone, titulo, cor, badge, atalho, children }) => {
    ...
    return (
      <div className="acc-sec" style={{ "--acc-color": cor || C.yellow }}>
        <button className="acc-hd" data-open={aberta}
                onClick={() => toggleSecao(id)} aria-expanded={aberta}>
          <span className="acc-ic"><IcoObra n={icone} s={17}/></span>
          <span className="acc-title">{titulo}</span>
          {badge != null && <span className="acc-badge">{badge}</span>}
          <span className="acc-chev"><ChevronBaixo s={14}/></span>
        </button>
        {aberta && (
          <div className="acc-body">
            ...
            {atalho && (
              <button className="acc-link" onClick={() => onTab(atalho)}>
                Abrir {atalho === "dre" ? "financeiro" : atalho} completo &rarr;
              </button>
            )}
          </div>
        )}
  ...
  return (
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:0}}>
      <button onClick={onVoltar} style={{background:"transparent",border:0,color:C.muted,
        ...
        fontFamily:"'Inter Display','Inter',sans-serif",fontWeight:700}}>&larr; Todas as obras</button>

      {/* Cabecalho da obra */}
      <div style={{background:`linear-gradient(135deg,${C.yellow} 0%,${C.yellowD} 100%)`,
                   color:C.ink,borderRadius:14,padding:"clamp(14px,4vw,20px)",marginBottom:11}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"flex-start"}}>
          <div style={{minWidth:0}}>
            {resumo.fase && (
              <p style={{fontSize:9.5,fontWeight:900,textTransform:"uppercase",letterSpacing:.8,opacity:.75}}>
                {resumo.fase.nome}
              </p>
            )}
            <h3 className="brk" style={{fontFamily:"'Inter Display','Inter',sans-serif",fontWeight:800,
                       fontSize:"clamp(19px,5.5vw,26px)",marginTop:2,lineHeight:1.15}}>{obra.name}</h3>
            {obra.engineer && (
              <p style={{fontSize:11.5,opacity:.8,marginTop:3}}>{obra.engineer}</p>
            )}
          </div>
          <Badge color={st.c}>{st.l}</Badge>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginTop:14}}>
          {[["Contrato", fmt(obra.contractValue)],
            ...
            ["Resultado", fmt(margem)]].map(([l,v])=>(
            <div key={l}>
              <p style={{fontSize:8.5,fontWeight:900,textTransform:"uppercase",opacity:.7,letterSpacing:.5}}>{l}</p>
              <p style={{fontSize:"clamp(12px,3.4vw,15px)",fontWeight:800,whiteSpace:"nowrap",
                         fontFamily:"'Inter Display','Inter',sans-serif"}}>{v}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {resumo.abaixoMin > 0 && (
        <div style={{background:`${C.red}0E`,border:`1px solid ${C.red}66`,borderRadius:9,
      ...
      {resumo.cmp.aReceber > 0.01 && (
        <div style={{background:`${C.orange}0E`,border:`1px solid ${C.orange}66`,borderRadius:9,
                     padding:"10px 12px",marginBottom:9}}>
          <p style={{fontSize:11.5,fontWeight:700,color:C.orange}}>
      ...
      {update && resumo.pedAbertos.length > 0 && (
        <div style={{background:`${C.blue}0C`,border:`1.5px solid ${C.blue}55`,
          ...
          <p style={{fontSize:10.5,color:C.muted,marginBottom:10,lineHeight:1.5}}>
            Tem {resumo.pedAbertos.length} pedido(s) a caminho. Ao confirmar a chegada,
            o material entra no estoque automaticamente.
          </p>
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {resumo.pedAbertos.map(p => {
              const forn = (data.fornecedores||[]).find(f => f.id === p.fornecedorId);
              const nItens = (p.itens||[]).filter(i => Number(i.qtd||0) > Number(i.qtdRecebida||0)).length;
              return (
                <div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                     gap:8,background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 11px"}}>
                  <div style={{minWidth:0,flex:1}}>
                    <p style={{fontSize:12,fontWeight:700,color:C.text}}>Pedido {p.numero}</p>
                    <p style={{fontSize:10,color:C.muted,marginTop:1}}>
                      {forn?.nome || "-"} - {nItens} item(ns) a receber
                    </p>
                  </div>
                  <Btn size="sm" onClick={() => setChegadaModal(p)}>Chegou</Btn>
                </div>
              );
            })}
          </div>
      ...
      {chegadaModal && (
        <Modal title={`Confirmar chegada - pedido ${chegadaModal.numero}`} onClose={() => setChegadaModal(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <p style={{fontSize:12.5,color:C.text,lineHeight:1.5}}>
              Confirma que <strong>todo o material</strong> deste pedido chegou na obra?
            </p>
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:9,padding:"10px 12px"}}>
              {(chegadaModal.itens||[]).filter(i => Number(i.qtd||0) > Number(i.qtdRecebida||0)).map(i => {
                const mat = (data.materiais||[]).find(m => m.id === i.materialId);
                const falta = Number(i.qtd||0) - Number(i.qtdRecebida||0);
                return (
                  <div key={i.id} style={{display:"flex",justifyContent:"space-between",gap:8,
                       padding:"4px 0",fontSize:12}}>
                    <span style={{color:C.text}}>{mat?.descricao || "Material"}</span>
                    <span style={{color:C.muted,fontWeight:700}}>{falta} {mat?.unidade || ""}</span>
                  </div>
                );
              })}
            </div>
            <p style={{fontSize:10.5,color:C.muted,lineHeight:1.5}}>
              Se chegou so uma parte, use a aba Compras para registrar o recebimento parcial.
            </p>
            <div style={{display:"flex",gap:8}}>
              <Btn v="ghost" onClick={() => setChegadaModal(null)} full>Ainda nao</Btn>
              <Btn onClick={() => confirmarChegada(chegadaModal)} full><Ic n="check"/> Sim, chegou tudo</Btn>
            </div>
          </div>
        </Modal>
      )}
      ...
      <Secao id="financeiro" icone="financeiro" titulo="Financeiro" cor={C.green} atalho="dre">
        ...
        <div style={{borderTop:`1px solid ${C.line}`,marginTop:7,paddingTop:7}}>
          <Par l="Resultado" v={fmt(margem)} c={margem >= 0 ? C.green : C.red}/>
        </div>
      </Secao>

      <Secao id="equipe" icone="equipe" titulo="Equipe"
             cor={C.blue} atalho="ponto"
             badge={resumo.presentesHoje > 0 ? resumo.presentesHoje : null}>
        <Par l="Lotados nesta obra" v={`${resumo.equipe}`}/>
        <Par l="Presentes hoje" v={`${resumo.presentesHoje}`}
      ...
      <Secao id="orcamento" icone="orcamento" titulo="Orcamento" cor={C.yellow} atalho="orc">
        {resumo.orc ? (<>
          ...
          {barra(resumo.orcTotal ? (resumo.cmp.comprado/resumo.orcTotal)*100 : 0,
      ...
      <Secao id="compras" icone="compras" titulo="Compras" cor={C.blue} atalho="cmp"
        ...
        <Par l="Pedidos em aberto" v={`${resumo.pedAbertos.length}`}
             c={resumo.pedAbertos.length ? C.orange : C.muted}/>
      </Secao>

      <Secao id="estoque" icone="estoque" titulo="Estoque" cor={C.purple} atalho="est"
                  ...
                  <div key={t.id} style={{ height: ALTURA_LINHA, position: "relative",
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: t.titulo ? C.subtle : corTarefa(t),
                        borderRadius: t.titulo ? 3 : 6,
                        cursor: t.titulo ? "default" : "grab",
                        // Caminho critico: borda vermelha grossa.
                        outline: critico.criticas.includes(t.id) ? `2px solid ${C.red}` : "none",
                        outlineOffset: 1,
      ...
      {tarefas.filter(t=>!t.titulo).length > 0 && (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", borderBottom: `1px solid ${C.line}`, overflowX: "auto" }}>
            {[["mensal","Financeiro mensal"],["curvaS","Curva S"],["ff","Fisico-financeiro"]].map(([id,lbl]) => (
...
function MiniFF({ label, v, c }) {
  return (
    <div style={{ flex: 1, minWidth: 100, background: C.surface, borderRadius: 9, padding: "8px 10px" }}>
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            {DIAS.map(([lbl, d]) => (
              <button key={d} onClick={() => toggleDia(d)} style={{
                flex: 1, minWidth: 42, padding: "9px 4px", borderRadius: 8, cursor: "pointer",
...
// ==============================================================
//  DIARIO DE OBRA (RDO) - conforme boa pratica
//  - Clima manha/tarde/noite
//  - Efetivo (mao de obra) + presencas do ponto sem trocar de aba
//  - Servicos executados: cada um atualiza o progresso da tarefa
//    do planejamento (alimenta a medicao de evolucao)
...
function DiarioObra({ data, update, showToast }) {
  ...
  const rdo = rdoExistente || {
    ...
    servicos: [], efetivo: [], presencas: [], equipamentos: [],
    ...
    <div className="anim" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <Bloco titulo="Ocorrencias e observacoes">
        <textarea value={rdo.ocorrencias} onChange={e => setOcorrencias(e.target.value)}
                  ...
                  rows={3} style={{ width: "100%", padding: "9px 11px", borderRadius: 8,
      ...
      <Bloco titulo={`Fotos${(rdo.fotos || []).length ? ` (${rdo.fotos.length})` : ""}`}>
        <div style={{ display: "grid", gridTemplateColumns: cols(2, 3, 4), gap: 8 }}>
          {(rdo.fotos || []).map(f => (
            <div key={f.url} style={{ position: "relative", borderRadius: 9, overflow: "hidden",
                                      border: `1px solid ${C.border}` }}>
          ...
          {/* Botao de adicionar */}
          <label style={{ height: 100, borderRadius: 9, border: `2px dashed ${C.border}`,
...
// Editor de servico executado: escolhe a tarefa e lanca o progresso acumulado.
function ModalServicoRDO({ servico, tarefas, jaLancados, onSalvar, onRemover, onClose }) {
  ...
  const salvar = () => {
    ...
    onSalvar({ tarefaId, etapaId: tarefa?.etapaId || "", descricao: tarefa?.nome || "",
               progressoAte: Math.max(0, Math.min(100, Number(prog) || 0)), obs });
  };
  ...
  return (
    <Modal title={servico ? "Servico executado" : "Novo servico"} onClose={onClose}>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {tarefa && tarefa.custo > 0 && (
          <div style={{ background: `${C.yellow}12`, border: `1px solid ${C.yellow}44`,
                        borderRadius: 9, padding: "8px 11px" }}>
            <p style={{ fontSize: 11, color: C.muted }}>Custo previsto desta tarefa</p>
            <p style={{ fontSize: 15, fontWeight: 800, color: C.yellowD }}>{fmt(tarefa.custo)}</p>
          </div>
        )}
        <div>
...
function MedicaoEvolucao({ data, update, showToast }) {
  ...
  if (!obras.length) {
    return <div style={{ padding: 24, textAlign: "center" }}>
      <p style={{ fontSize: 13, color: C.muted }}>Cadastre uma obra para medir a evolucao.</p>
    </div>;
  }
  ...
  return (
    <div className="anim" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* Resumo */}
      <div style={{ display: "grid", gridTemplateColumns: cols(2, 4, 4), gap: 8 }}>
        <MiniKpi label="Avanco fisico" value={`${resumo.avancoFisico.toFixed(0)}%`} cor={C.blue} />
        <MiniKpi label="Servicos" value={`${resumo.total}`} cor={C.text} />
        <MiniKpi label="Medidos pelo diario" value={`${resumo.peloDiario}`} cor={C.green}
                 sub={`${resumo.total - resumo.peloDiario} manuais`} />
        <MiniKpi label="Dias trabalhados" value={`${resumo.diasTotais}`} cor={C.yellow}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
        ...
        {tarefas.length === 0 ? (
          <p style={{ padding: 20, fontSize: 12, color: C.muted, textAlign: "center" }}>
            Sem servicos no planejamento desta obra.
          </p>
        ) : tarefas.map(t => (
          <div key={t.id} style={{ padding: "11px 14px", borderTop: `1px solid ${C.line}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {editando === t.id ? (
                <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                  <input type="number" defaultValue={t.progresso} min="0" max="100"
                         id={`prog-${t.id}`} style={{ width: 60, padding: "6px", borderRadius: 6,
                         border: `1px solid ${C.border}`, fontSize: 12, textAlign: "center" }} />
                  <Btn size="sm" onClick={() => {
                    const v = document.getElementById(`prog-${t.id}`).value;
                    ajustarManual(t.id, v); setEditando(null);
                  }}><Ic n="check" /></Btn>
                </div>
              ) : (
                <button onClick={() => setEditando(t.id)} style={{ display: "flex", alignItems: "center",
                        gap: 5, background: "transparent", border: 0, cursor: "pointer", flexShrink: 0 }}>
                  <span style={{ fontSize: 16, fontWeight: 800,
                                 color: t.progresso >= 100 ? C.green : C.blue }}>{t.progresso}%</span>
                  <Ic n="edit" s={13} color={C.muted} />
                </button>
              )}
            </div>
            <div style={{ marginTop: 7, height: 6, background: C.line, borderRadius: 99, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${t.progresso}%`,
                            background: t.progresso >= 100 ? C.green : C.blue, borderRadius: 99 }} />
            </div>
          </div>
        ))}
      </div>
...
function Obsoletos({ data, update, showToast, onTab }) {
  ...
  const obrasInativas = (data.obras || []).filter(o => o.status === "done" || o.status === "paused");
  const equipeInativa = (data.employees || []).filter(e => e.active === false);
  ...
  return (
    <div className="anim" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", gap: 6 }}>
        {[["obras", `Obras (${obrasInativas.length})`], ["equipe", `Equipe (${equipeInativa.length})`]].map(([id, lbl]) => (
          <button key={id} onClick={() => setVisao(id)} style={{
            ...
            color: visao === id ? C.yellowD : C.muted, fontSize: 12.5, fontWeight: 700,
          }}>{lbl}</button>
        ))}
      </div>

      <p style={{ fontSize: 11, color: C.muted, lineHeight: 1.5, padding: "0 2px" }}>
        Aqui ficam obras concluidas ou pausadas e funcionarios desligados. Nada foi
        apagado - ponto, DRE e contratos seguem preservados para consulta.
      </p>
      ...
      {visao === "obras" && (
        obrasInativas.length === 0
          ...
          : (
            <div style={{ display: "grid", gridTemplateColumns: cols(1, 1, 2), gap: 10 }}>
              {obrasInativas.map(o => {
                ...
                return (
                  <div key={o.id} style={{ background: C.card, border: `1px solid ${C.border}`,
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start" }}>
                      <div style={{ minWidth: 0 }}>
                        <p className="brk" style={{ fontSize: 14, fontWeight: 800, color: C.text,
                                   fontFamily: "'Inter Display','Inter',sans-serif" }}>{o.name}</p>
                        {o.address && <p style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{o.address}</p>}
                      </div>
                      <Badge color={st.c}>{st.l}</Badge>
                    </div>
                    <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                      {o.contractValue > 0 && (
                        <span style={{ fontSize: 11, color: C.muted }}>Contrato {fmt(o.contractValue)}</span>
                      )}
                      {o.engineer && <span style={{ fontSize: 11, color: C.muted }}>{o.engineer}</span>}
                    </div>
                    <div style={{ display: "flex", gap: 6, marginTop: 11 }}>
                      <Btn v="ghost" size="sm" onClick={() => { onTab?.("obras"); }}>Ver dados</Btn>
                      <Btn v="ghost" size="sm" onClick={() => reativarObra(o.id)}>Reativar</Btn>
                    </div>
                  </div>
                );
              })}
      ...
      {visao === "equipe" && (
        equipeInativa.length === 0
          ...
          : (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
              {equipeInativa.map(e => (
                <div key={e.id} style={{ padding: "12px 14px", borderBottom: `1px solid ${C.line}`,
                     display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <p className="brk" style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{e.name}</p>
                    <div style={{ display: "flex", gap: 8, marginTop: 2, flexWrap: "wrap" }}>
                      {e.role && <span style={{ fontSize: 10.5, color: C.muted }}>{e.role}</span>}
                      {e.endDate && <span style={{ fontSize: 10.5, color: C.muted }}>Saida {fmtDate(e.endDate)}</span>}
...
function VazioObs({ texto }) {
  return (
    <div style={{ padding: 28, textAlign: "center", background: C.card,
...
function Cadastros({ data, update, showToast, onTab }) {
  ...
  const salvarCategoria = (f) => {
    ...
    // Chave: mantem a existente ou cria um slug estavel a partir do nome.
    const v = f.v || nome.toLowerCase().normalize("NFD").replace(/[^a-z0-9]/g, "").slice(0, 20) || uid();
    const jaExiste = atuais.some(c => c.v === v && c.v !== f.v);
    if (jaExiste) { showToast("Já existe uma categoria com esse nome.", "error"); return; }
    // Parte das padrao se o operador ainda nao tinha lista propria.
  ...
  const excluirCategoria = (c) => {
    ...
    if (emUso) {
      showToast(`"${c.l}" está em uso por ${emUso} lançamento(s). Reclassifique antes.`, "error");
      return;
    }
    ...
    <button
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
        ...
        <div style={{textAlign:"right",flexShrink:0}}>
          ...
          {atalho && <p style={{fontSize:9,color:C.muted,marginTop:2}}>abrir &rarr;</p>}
        </div>
      </div>
    </button>
  );
  ...
  const Linha = ({ titulo, sub, onEdit, onDel }) => (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,
      ...
      <div style={{display:"flex",gap:4,flexShrink:0}}>
        ...
        {onDel && <Btn size="sm" v="danger" onClick={onDel}>x</Btn>}
      </div>
    </div>
  );
    ...
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      {sec === "menu" && (<>
        ...
        <div style={{display:"grid",gridTemplateColumns:cols(1,2,2),gap:9}}>
          ...
          <Card id="categorias" icone="" titulo="Categorias do DRE"
                qtd={categoriasDesp(data).length}
                sub="como as despesas se agrupam no resultado"/>
        </div>
      ...
      {sec === "fases" && (<>
        ...
        <Btn v="ghost" onClick={()=>onTab("obras")} full>Abrir o quadro de obras &rarr;</Btn>
      </>)}
      ...
      {sec === "categorias" && (<>
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px"}}>
          <p style={{fontSize:11,color:C.muted,lineHeight:1.55}}>
            Como as despesas se agrupam no DRE. Crie as suas (ex.: <strong style={{color:C.text}}>Combustivel</strong>,
            <strong style={{color:C.text}}> EPI</strong>, <strong style={{color:C.text}}>Ferramentas</strong>).
            Categoria em uso nao pode ser apagada - reclassifique antes.
          </p>
        </div>
        <Btn onClick={()=>setCatModal({ v:"", l:"" })} full><Ic n="plus"/> Nova categoria</Btn>
        {categoriasDesp(data).map(c => {
          const emUso = (data.outrasDesp||[]).filter(d=>d.categoria===c.v).length
                      + (data.despesasEmpresa||[]).filter(d=>d.categoria===c.v).length;
          return (
            <div key={c.v} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,
                                   padding:"10px 12px",display:"flex",alignItems:"center",gap:9}}>
              <p style={{fontSize:12.5,fontWeight:700,color:C.text,flex:1,minWidth:0}}>{c.l}</p>
              {emUso > 0 && <span style={{fontSize:10.5,color:C.muted,flexShrink:0}}>{emUso} lanc.</span>}
              <button onClick={()=>setCatModal({ v:c.v, l:c.l })} style={{background:"transparent",
                      border:0,color:C.muted,cursor:"pointer",padding:4}}><Ic n="edit" s={14}/></button>
              {c.v !== "outros" && (
                <button onClick={()=>excluirCategoria(c)} style={{background:"transparent",
                        border:0,color:C.red,cursor:"pointer",padding:4}}><Ic n="trash" s={14}/></button>
              )}
            </div>
          );
        })}
      ...
      {/*  MODAIS  */}
      {catModal && (
        <Modal title={catModal.v ? "Editar categoria" : "Nova categoria"} onClose={()=>setCatModal(null)}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <Inp label="Nome da categoria *" value={catModal.l}
                 onChange={v=>setCatModal(f=>({...f,l:v}))} placeholder="Ex.: Combustivel"/>
            <div style={{display:"flex",gap:8}}>
              <Btn v="ghost" onClick={()=>setCatModal(null)} full>Cancelar</Btn>
              <Btn onClick={()=>salvarCategoria(catModal)} full><Ic n="check"/> Salvar</Btn>
            </div>
          </div>
        </Modal>
      )}
...
function Estoque({ data, update, showToast }) {
  ...
  //  Executar serviço &rarr; baixa automática 
...
const CAIXA_CATS = [
  ...
  { v:"servico",      l:"Serviço avulso",         icon:"" },
      ...
      {obra && (<>
        ...
        <div style={{background:caixa.saldo>=0?`${C.green}08`:`${C.red}08`,border:`1.5px solid ${caixa.saldo>=0?C.green:C.red}`,borderRadius:10,padding:"16px 18px",textAlign:"center"}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:20,marginTop:6}}>
            ...
            <p style={{fontSize:12,color:C.muted}}>v Gastos: <strong style={{color:C.red}}>{fmt(caixa.totalDespesas)}</strong></p>
          </div>
        </div>
      ...
      {modal&&obra&&(
        <Modal title={tipoNovo==="aporte"?"Registrar aporte do cliente":"Registrar gasto"} onClose={()=>setModal(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {/* Toggle tipo */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
              {[["aporte","^ Aporte",C.green],["despesa","v Gasto",C.red]].map(([v,l,c])=>(
    ...
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <div style={{background:C.bg,border:`1.5px solid ${C.border}`,borderRadius:10,overflow:"hidden",boxShadow:`0 1px 4px ${C.shadow}`}}>
        ...
        <div style={{padding:"8px 14px"}}>
          ...
          {dre.totalDeducoes===0&&<DRow label="Configure as alíquotas em Ajustes &rarr; DRE" value={0} color={C.muted} indent={1}/>}
...
const NAV_GROUPS = [
  ...
  {
    id: "obsoletos_grp", label: "Obsoletos", icon: "file", color: C.muted,
    tabs: ["obsoletos"],
  },
  {
...
const TAB_META = {
  ...
  resc:   { label: "Rescisão",   icon: "file",     group: "fin_grp"  },
  ...
  est:      { label: "Estoque",     icon: "home",   group: "compras_grp" },
  cmp:      { label: "Compras",     icon: "dollar", group: "compras_grp" },
  ...
  obsoletos: { label: "Obsoletos", icon: "file",   group: "obsoletos_grp" },
};
...
export default function App() {
  ...
  const SIDEBAR_W = 224;
  // No desktop a navegação vira sidebar &rarr; não há barra inferior ocupando espaço
        ...
        {isDesktop && (
          <aside className="no-print" style={{
            ...
            {/* Navegação: grupos colapsáveis. Clique no cabeçalho abre/fecha;
                o grupo da aba atual já começa aberto. O trilho na cor do grupo
                dá o senso de nível - filho pende do pai. */}
            <nav style={{ flex:1, overflowY:"auto", padding:"10px 8px" }}>
              {visibleGroups.map(group => {
                ...
                const vars = {
                  "--gc":      group.color,
                  "--gc-hov":  `${group.color}0F`,
                  "--gc-act":  `${group.color}18`,
                ...
                return (
                  <div key={group.id} style={{ marginBottom:2, ...vars }}>
                    {/* Cabeçalho clicável. Um grupo fechado que contém a aba
                        atual mostra um ponto na sua cor - você não perde de
                        vista onde está, mesmo com o grupo recolhido. */}
                    <button
                      className="nav-grp-hd"
                      data-open={aberto || contémAtiva}
                      onClick={() => toggleGrupo(group.id)}
                      aria-expanded={aberto}
                    >
                      <Ic n={group.icon} s={13}/>
                      <span>{group.label}</span>
                      {badge && <span className="nav-dot" style={{ marginLeft:"auto" }}/>}
                      {!badge && !aberto && contémAtiva &&
                        <span style={{ marginLeft:"auto", width:5, height:5, borderRadius:"50%",
                                       background:"var(--gc)" }}/>}
                      {!badge && !aberto && !contémAtiva &&
                        <span className="nav-count">{group.tabs.length}</span>}
                      <span className="nav-chev" style={{ marginLeft:(badge||( !aberto)) ? 8 : "auto" }}>&rsaquo;</span>
                    </button>

                    {/* Corpo - só monta quando aberto */}
                    {aberto && (
                      <div className="nav-body">
                        {group.tabs.map(tabId => {
                          const meta = TAB_META[tabId];
                          const ativo = tab === tabId;
                          const temPend = tabBadge?.[tabId];
                          return (
                            <button
                              key={tabId}
                              className="nav-item"
                              data-active={ativo}
                              onClick={() => { setObraAberta(""); setTab(tabId); abrirGrupoDaAba(tabId); }}
                              aria-current={ativo ? "page" : undefined}
                            >
                              <Ic n={meta.icon} s={13}/>
                              <span>{meta.label}</span>
                              {temPend && <span className="nav-dot"/>}
                            </button>
                          );
                        })}
                      </div>
