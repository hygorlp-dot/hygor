...
function ObraDetalhe({ data, obraId, onVoltar, onTab }) {
  ...
  const Bloco = ({ titulo, atalho, children }) => (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
        <p style={{fontSize:10,fontWeight:900,color:C.muted,textTransform:"uppercase",letterSpacing:.8}}>
          {titulo}
        </p>
        {atalho && (
          <button onClick={()=>onTab(atalho)} style={{background:"transparent",border:0,color:C.yellow,
            fontSize:10,fontWeight:700,cursor:"pointer"}}>abrir &rarr;</button>
        )}
      </div>
      {children}
    </div>
  ...
  const Par = ({ l, v, c }) => (
    <div style={{display:"flex",justifyContent:"space-between",gap:8,marginTop:3}}>
      ...
      <span style={{fontSize:11.5,fontWeight:700,color:c||C.text,whiteSpace:"nowrap"}}>{v}</span>
    </div>
  );

  return (
    <div className="anim" style={{display:"flex",flexDirection:"column",gap:11}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.yellow} 0%,${C.yellowD} 100%)`,
                   color:C.ink,borderRadius:14,padding:"clamp(13px,4vw,20px)"}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:8,alignItems:"flex-start"}}>
          <div style={{minWidth:0}}>
            {resumo.fase && (
            ...
            <h3 className="brk" style={{fontFamily:"'Inter Display','Inter',sans-serif",fontWeight:800,
                       fontSize:"clamp(18px,5.5vw,26px)",marginTop:2,lineHeight:1.15}}>{obra.name}</h3>
            {obra.engineer && (
      ...
      {/* Alertas */}
      {resumo.abaixoMin > 0 && (
        <div style={{background:`${C.red}0E`,border:`1px solid ${C.red}66`,borderRadius:8,padding:"9px 11px"}}>
          <p style={{fontSize:11.5,fontWeight:700,color:C.red}}>
            ! {resumo.abaixoMin} material(is) abaixo do estoque mínimo
          </p>
        </div>
      )}
      {resumo.cmp.aReceber > 0.01 && (
        <div style={{background:`${C.orange}0E`,border:`1px solid ${C.orange}66`,borderRadius:8,padding:"9px 11px"}}>
          <p style={{fontSize:11.5,fontWeight:700,color:C.orange}}>
             {fmt(resumo.cmp.aReceber)} comprado e ainda não entregue
          </p>
        </div>
      )}

      <div style={{display:"grid",gridTemplateColumns:cols(1,2,2),gap:10}}>

        <Bloco titulo="Equipe" atalho="ponto">
        ...
        <Bloco titulo="Financeiro" atalho="dre">
        ...
        <Bloco titulo="Orçamento" atalho="orc">
          {resumo.orc ? (<>
            <Par l="Custo orçado" v={fmt(resumo.orcTotal)}/>
            <Par l="Já comprado" v={fmt(resumo.cmp.comprado)} c={C.blue}/>
            <Par l="Saldo" v={fmt(resumo.orcTotal - resumo.cmp.comprado)}
                 c={resumo.cmp.comprado > resumo.orcTotal ? C.red : C.green}/>
            <div style={{marginTop:8,height:5,background:C.line,borderRadius:99,overflow:"hidden"}}>
              <div style={{height:"100%",
                width:`${Math.min(100, resumo.orcTotal ? (resumo.cmp.comprado/resumo.orcTotal)*100 : 0)}%`,
                background: resumo.cmp.comprado > resumo.orcTotal ? C.red : C.yellow, borderRadius:99}}/>
            </div>
          </>) : (
            <p style={{fontSize:11,color:C.muted,lineHeight:1.5}}>
        ...
        <Bloco titulo="Compras" atalho="cmp">
        ...
        <Bloco titulo="Estoque" atalho="est">
        ...
        <Bloco titulo="Medições" atalho="medicoes">
      ...
      {/* Atividade recente */}
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px"}}>
        <p style={{fontSize:10,fontWeight:900,color:C.muted,textTransform:"uppercase",
                   letterSpacing:.8,marginBottom:9}}>O que aconteceu</p>
        {atividade.length === 0
          ? <p style={{fontSize:11.5,color:C.muted}}>Nenhuma movimentação registrada ainda.</p>
          : atividade.map((e,i) => (
            <div key={i} style={{display:"flex",gap:9,paddingTop:7,marginTop:7,
                                 borderTop: i ? `1px solid ${C.line}` : 0, alignItems:"baseline"}}>
              <span style={{fontSize:13,flexShrink:0}}>{e.tipo}</span>
              <div style={{flex:1,minWidth:0}}>
