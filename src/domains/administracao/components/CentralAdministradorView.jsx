...
export default function CentralAdministrador({data,update,showToast,currentUser}){
  ...
  return <div className="anim" style={{display:"flex",flexDirection:"column",gap:12}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:7}}><Btn v={secao==="auditoria"?"primary":"ghost"} onClick={()=>setSecao("auditoria")}><Ic n="clipboard"/> Auditoria e resumo por IA</Btn><Btn v={secao==="usuarios"?"primary":"ghost"} onClick={()=>setSecao("usuarios")}><Ic n="users"/> Usuários e permissões</Btn><Btn v={secao==="aprovacoes"?"primary":"ghost"} onClick={()=>setSecao("aprovacoes")}><Ic n="check"/> Aprovações</Btn><Btn v={secao==="bases"?"primary":"ghost"} onClick={()=>setSecao("bases")}><Ic n="fileText"/> Bases de preço</Btn></div>
