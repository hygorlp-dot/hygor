...
import { cancelThirdPartyMeasurement, createThirdPartyMeasurement, createThirdPartyPayment, payThirdPartyMeasurement, reverseThirdPartyPayment } from "./third-party-payment-mutations";
...
describe("pagamentos auditáveis de terceiros", () => {
  ...
  it("faz a medição aprovada reconhecer custo/obrigação e o pagamento apenas liquidar", () => {
    const measured=createThirdPartyMeasurement({data:{medicoesTerc:[]},actor,id:"m-1",measurement:{tercId:"t-1",obraId:"o-1",data:"2026-07-05",numero:1,total:800,itens:[{etapaId:"e-1",valor:800,pctAcum:50}]}});
    const before=buildFinancialLedger(measured);
    expect(selectDRE(before,{obraId:"o-1",competence:"2026-07"}).costCents).toBe(80000);
    const paid=payThirdPartyMeasurement({data:measured,measurementId:"m-1",actor,id:"p-1",payment:{tercName:"Pedreiro",date:"2026-07-06",pagador:"obra"}});
