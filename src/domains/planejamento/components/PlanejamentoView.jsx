...
export default function Planejamento({ data, update, showToast, obraIdFixo="", currentUser=null, dispatchCommand=null }) {
  ...
  const abrirNovoCompromisso = () => {
    ...
    setNovoCompromisso({descricao:"",activityId:primeiraAtividade.id,quantidadePrometida:"",data:today(),criticalActivity:false,blockingReason:"",requiredTrainings:[]});
  };
  const alternarTreinamentoExigido = key => setNovoCompromisso(form=>({...form,requiredTrainings:(form.requiredTrainings||[]).includes(key)?form.requiredTrainings.filter(item=>item!==key):[...(form.requiredTrainings||[]),key]}));
  ...
  const abrirNovoAvanco = compromisso => setNovoAvanco({commitmentId:compromisso.id,activityId:compromisso.activityId,descricao:compromisso.descricao||"",quantity:"",data:today(),workerIds:[],criticalActivity:Boolean(compromisso.criticalActivity||compromisso.atividadeCritica),requiredTrainings:compromisso.requiredTrainings||[]});
  ...
  const registrarAvanco = async () => {
    ...
    const result=await dispatchCommand(()=>({type:OPERATIONAL_COMMAND.PROGRESS_RECORD_SAVED,idempotencyKey:`avanco-fisico-${uid()}`,
      actorId:currentUser?.id||"",actorName:currentUser?.nome||"",expectedVersion:0,payload:{record:{id:uid(),obraId,activityId:form.activityId,commitmentId:form.commitmentId,quantity:Number(form.quantity),data:form.data,workerIds:form.workerIds||[],criticalActivity:Boolean(form.criticalActivity),requiredTrainings:form.requiredTrainings||[]}}}));
