...
describe("planejamento",()=>{
 ...
 it("calcula PPC e causa",()=>{const ppc=calculatePPC([{status:"concluido"},{status:"nao_concluido",motivoNaoCumprimento:"material"},{status:"bloqueado"}]);expect(ppc.ppc).toBe(.5);expect(ppc.blocked).toBe(1);expect(ppc.causes[0].cause).toBe("material");});
